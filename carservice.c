/*
    ...
    This program takes in the type of car and its' mileage, then prompts the
   user for which services they need and adds the cost to the price if they want
   the service, the program then adds tax and prints out the total cost of all
   services.
    */
#include <stdio.h>
#include <string.h>
...
int main() {
    ...
    const char serviceTextCar[8][50] = {"Oil Change $89.95?",
                                        "Tire alignment $50.00?",
                                        "Windshield wiper blades? $45.95",
                                        "Engine air filter? $30.50",
                                        "Brake fluid? $73.00",
                                        "Timing belt? $245.00",
                                        "Transmission fluid? $75.00"};
    const char serviceTextTruck[8][50] = {"Oil Change $119.95?",
                                          "Tire alignment $100.00?",
                                          "Windshield wiper blades? $45.95",
                                          "Engine air filter? $40.50",
                                          "Brake fluid? $104.00",
                                          "Timing belt? $345.00",
                                          "Transmission fluid? $85.00"};
    const double carPrices[] = {89.95, 50.00,  45.95, 30.50,
                                73.00, 245.00, 75.00};
    const double truckPrices[] = {119.95, 100.00, 45.95, 40.50,
                                  104.00, 345.00, 85.00};
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (servicesNeeded[i] == 1) {
            ...
            scanf(" %c", &userChoice);
            if (userChoice != 'y' && userChoice != 'Y' && userChoice != 'n' &&
                userChoice != 'N') {
                printf("Invalid choice entered.\n");
                printf("Goodbye.\n");
                return 3;
